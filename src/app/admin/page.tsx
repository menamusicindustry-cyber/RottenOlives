...
export default function AdminPage() {
  ...
  async function doLogin(e: React.FormEvent) {
    e.preventDefault();
    setLoading("login"); setError(null);
    try {
      ...
      const j = await r.json();
      if (!r.ok || j.ok === false) throw new Error(j?.error || "Login failed");
      setAuthed(true);
      setPassword("");
    } catch (e: any) {
  ...
  async function doPreview() {
    if (!origin) return;
    setLoading("preview"); setError(null); setImportResult(null);
    try {
      const url = `${origin}/api/spotify/import/preview?playlistId=${encodeURIComponent(playlistId)}${market ? `&market=${encodeURIComponent(market)}` : ""}`;
      log(`GET ${url}`);
      const r = await fetch(url, { cache: "no-store" });
      const j = await r.json();
      log(`preview → ${r.status} ${JSON.stringify(j).slice(0, 400)}${JSON.stringify(j).length > 400 ? "…" : ""}`);
      ...
      setError(msg); setPreview(null); log(`preview ERROR → ${msg}`);
    } finally { setLoading(null); }
  }

  async function doImport() {
    if (!origin) return;
    setLoading("import"); setError(null); setImportResult(null);
    try {
      ...
      const j = await r.json();
      log(`import → ${r.status} ${JSON.stringify(j).slice(0, 400)}${JSON.stringify(j).length > 400 ? "…" : ""}`);
      if (!r.ok || j.ok === false) throw new Error(j?.error || "Import failed");
      setImportResult({ imported: j.imported || 0, items: j.items || [] });
    } catch (e: any) {
      const msg = String(e?.message || e);
      setError(msg); log(`import ERROR → ${msg}`);
    } finally { setLoading(null); }
  }
  ...
  return (
    <div className="section" style={{ display: "grid", gap: 16 }}>
      <header className="card col" style={{ gap: 8 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div className="meta">{authed ? "Logged in" : "Please enter your admin password"}</div>
          </div>
          {authed && (
            <button onClick={doLogout} className="px-3 py-2 rounded-full text-white" style={{ backgroundColor: "#6b7280" }}>
              Logout
            </button>
          )}
        </div>
      ...
      {!authed ? (
        <form onSubmit={doLogin} className="card col" style={{ gap: 12, padding: 16, maxWidth: 420, margin: "0 auto" }}>
                    ...
                    <div className="grid" style={{ gap: 8 }}>
                      {preview.items.map((it) => (
                        <div key={it.spotifyTrackId} className="card" style={{ display: "grid", gridTemplateColumns: "56px 1fr", gap: 12, alignItems: "center" }}>
                          {it.coverUrl ? (
                            <img src={it.coverUrl} alt={it.title} width={56} height={56} style={{ borderRadius: 8, objectFit: "cover" }} />
                          ) : (
                          ...
                          <div className="col" style={{ gap: 2 }}>
                            ...
                            <div className="meta" style={{ wordBreak: "break-all" }}>track: {it.spotifyTrackId}</div>
                          </div>
                        </div>
                      ))}
                    </div>
